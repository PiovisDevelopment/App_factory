...
class HybridDiscovery:
    ...
    def __init__(self, plugins_dir: str | Path = "./plugins", config_dir: str | Path = "./config"):
        """
    ...
    def _load_prefixes(self) -> None:
        ...
        self._prefix_pattern = re.compile(rf"^({prefix_list})_[a-z0-9_]+_plugin$")
    ...
    def discover_plugin(self, plugin_path: Path) -> DiscoveredPlugin:
        ...
        if name != folder_name:
            all_errors.append(f"Manifest name '{name}' does not match folder name '{folder_name}'")

        # Cross-validate: prefix should match contract
        if prefix and prefix != contract:
            all_errors.append(f"Folder prefix '{prefix}' does not match manifest contract '{contract}'")

        # Validate contract is known
        if contract not in self._prefixes:
            all_errors.append(
                f"Unknown contract type '{contract}'. Valid types: {', '.join(sorted(self._prefixes.keys()))}"
            )
    ...
    def scan_by_contract(self, contract: str, include_invalid: bool = False) -> list[DiscoveredPlugin]:
        """
...
def discover_plugins(
    plugins_dir: str | Path = "./plugins", config_dir: str | Path = "./config", include_invalid: bool = False
