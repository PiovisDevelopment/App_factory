...
# Import DiscoveredPlugin for type hints (avoid circular import at runtime)
...
class PluginLoader:
    ...
    def __init__(self, auto_install_deps: bool = False, pip_executable: str = "pip"):
        """
    ...
    def import_module(self, plugin_path: Path, entry_point: str, force_reload: bool = False) -> Any | None:
        """
    ...
    def find_plugin_class(self, module: Any, contract: str) -> type[PluginBase] | None:
        """
    ...
    def load(self, discovered: "DiscoveredPluginType", force_reload: bool = False) -> LoadedPlugin | None:
        """
        ...
        if dependencies:
            ...
            if missing:
                if self.auto_install_deps:
                    ...
                    load_errors.append(f"Missing dependencies: {missing}. Run: pip install {' '.join(missing)}")
                    # Continue anyway, might work with optional deps

        # Import module
        module = self.import_module(discovered.path, discovered.entry_point, force_reload=force_reload)
    ...
    def unload(self, loaded: LoadedPlugin) -> bool:
        ...
        """
        module_key = (
            f"{loaded.path.name}.{loaded.manifest.entry_point if hasattr(loaded.manifest, 'entry_point') else 'plugin'}"
        )
    ...
    def reload(self, discovered: "DiscoveredPluginType") -> LoadedPlugin | None:
        """
...
async def initialize_plugin(loaded: LoadedPlugin, config: dict[str, Any]) -> bool:
    """
