...
MethodHandler = Callable[
    [dict[str, Any] | None, str | int | None],
...
class JsonRpcRouter:
    ...
    def _register_builtin_methods(self) -> None:
        ...
        # plugin/get_config_schema - get JSON schema for plugin config
        async def handle_plugin_config_schema(params, id):
            """
            Get JSON Schema for a plugin's configuration.

            Returns Pydantic-generated schema for dynamic frontend forms.

            Args:
                params: {"plugin_id": "stt_whisper"} or {"plugin_id": "tts_kokoro"}

            Returns:
                JSON Schema dictionary for the plugin's config model.
            """
            from .config_models import get_config_schema

            plugin_id = params.get("plugin_id") if params else None
            if not plugin_id:
                raise ValueError("Missing 'plugin_id' parameter")

            try:
                return get_config_schema(plugin_id)
            except ValueError as e:
                raise RuntimeError(str(e))

        self._methods["plugin/get_config_schema"] = MethodRegistration(
            handler=handle_plugin_config_schema,
            description="Get JSON Schema for plugin configuration"
        )
    ...
    async def _invoke_plugin_method(
        ...
        loaded = None
        for _name, plugin in self.manager.loaded_plugins.items():
