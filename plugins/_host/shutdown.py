...
class ShutdownHandler:
    ...
    def __init__(
        ...
        logger.debug(
            f"ShutdownHandler initialized: shutdown_timeout={shutdown_timeout}s, "
    ...
    def _atexit_cleanup(self) -> None:
        ...
        if self._in_flight:
            logger.warning(
                f"Exiting with {len(self._in_flight)} in-flight requests"
            )
    ...
    async def wait_for_in_flight(self) -> bool:
        ...
        if self._in_flight:
            logger.warning(
                f"Timeout: {len(self._in_flight)} requests still in-flight"
            )
            return False
...
class ShutdownContext:
    ...
    def __init__(self, handler: ShutdownHandler, request_id: Optional[int] = None):
        """
        Initialize context.
