...
async def test_ollama_llm():
    ...
    try:
        ...
        messages = [Message(role=MessageRole.USER, content="What is 2+2? Reply with just the number.")]

        options = CompletionOptions(temperature=0.0, max_tokens=50)

        print("  Generating response (this may take a moment)...")
        result = await plugin.complete(messages, options)

        print("  OK Completion received!")
