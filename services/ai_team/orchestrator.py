...
# --- Logging to Tauri via stderr ---
def emit_log(message: str, level: str = "info", agent: str = "orchestrator") -> None:
    """
    Emit a structured log that can be captured by Tauri and streamed to the UI.
    ...
    # Print to stderr to avoid corrupting JSON-RPC stdout
...
class ResearchNode(BaseNode[AgentState]):
    ...
    async def run(self, ctx: GraphRunContext[AgentState]) -> DesignNode:
        emit_log(f"Researching: {ctx.state.user_prompt[:50]}...", "info", "researcher")
        ...
        emit_log("Research complete", "success", "researcher")
...
class DesignNode(BaseNode[AgentState]):
    ...
    async def run(self, ctx: GraphRunContext[AgentState]) -> DeveloperNode:
        emit_log("Creating design blueprint...", "info", "designer")
        ...
        emit_log("Design blueprint complete", "success", "designer")
...
class DeveloperNode(BaseNode[AgentState]):
    ...
    async def run(self, ctx: GraphRunContext[AgentState]) -> QANode:
        emit_log("Writing code from blueprint...", "info", "developer")
        ...
        emit_log("Code generation complete", "success", "developer")
...
class QANode(BaseNode[AgentState]):
    ...
    async def run(self, ctx: GraphRunContext[AgentState]) -> End[AgentState]:
        emit_log("Running QA validation...", "info", "qa")
        ...
        emit_log("QA validation complete", "success", "qa")

        return End(ctx.state)

# --- Graph Definition ---
workflow_graph = Graph(
    nodes=[ResearchNode, DesignNode, DeveloperNode, QANode],
...
async def run_workflow_api(user_prompt: str) -> AgentState:
    ...
    """
    emit_log(f"Starting workflow: {user_prompt[:50]}...", "info", "orchestrator")
    ...
    emit_log("Workflow completed successfully", "success", "orchestrator")
    return final_state
