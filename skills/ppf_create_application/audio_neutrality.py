...
def validate_audio_ui_neutrality(files: Iterable[str], audio_intent: bool) -> AudioUiValidationResult:
    """
    ...
    for file_path in files:
        ...
        for idx, line in enumerate(lines, start=1):
            for pattern in BLOCKLIST_PATTERNS:
                if pattern in line:
                    violations.append(AudioUiViolation(file=path, line_number=idx, pattern=pattern, line=line.strip()))
